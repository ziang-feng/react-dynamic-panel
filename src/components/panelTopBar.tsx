...
export function PanelTopBar({ panelID }: { panelID: PanelID }) {
    ...
    const dragEnterCallback = (_e: MouseEvent,) => {
        ...
        if (workspaceProps!.pageDataReference[workspaceDragProps!.draggedData!.pageID].locked) {
            // if dragged page is locked and this is a different panel, do not allow drop
            if (workspaceDragProps!.draggedData!.panelID != panelID) {
                workspaceDragProps!.setDragCursorStyle("");
                return;
            }
            // if dragged page is locked, and the last page is not locked, ordering rule will be violated, do not allow drop
            const panelPageList = workspaceProps!.panelPageListReference[panelID];
            const lastPageID = panelPageList[panelPageList.length - 1];
            if (!workspaceProps!.pageDataReference[lastPageID].locked) {
                workspaceDragProps!.setDragCursorStyle("");
                return;
            }
        }
    ...
    // if focused content is changed or page list is changed, scroll to show the focused tab
    useEffect(() => {
    ...
    }, [workspaceProps!.panelFocusReference[panelID], workspaceProps!.panelPageListReference[panelID]]);
    ...
    return (
        <div className="flex flex-row h-10 flex-shrink-0 flex-grow-0 bg-background-low-1 relative w-full max-w-full z-40" onMouseDownCapture={(e) => { e.preventDefault() }}>
            <div className="h-10 flex-shrink-0 relative" style={{ width: "calc(100% - 5rem)" }} >
                <div className={`overflow-x-auto flex-wrap flex flex-col content-start relative h-full scrollbar-hide overscroll-x-none ${workspaceDragProps!.draggedData && isDragOverEmptyArea ? "bg-background-high-2" : ""}`} onWheel={(e) => { pageTabListScrollHandler(e.nativeEvent, pageTabListContainerRef) }} ref={pageTabListContainerRef}>
                    {pageTabJSXList}
                </div>
            </div>
            {panelTabListButton}
...
function PanelDropDownMenu({ panelID, isPanelMenuOpen, setIsPanelMenuOpen, panelMenuButtonRef }: { panelID: PanelID, isPanelMenuOpen: boolean, setIsPanelMenuOpen: Dispatch<boolean>, panelMenuButtonRef: RefObject<HTMLButtonElement> }) {
    ...
    return <div className={`flex-col absolute border shadow bg-background border-background-low-2 rounded top-10 right-0 w-52 overflow-hidden ${isPanelMenuOpen ? "flex" : "hidden"}`} ref={selfRef}>
        <button type="button" className="flex px-2 h-10 bg-background text-foreground hover:text-foreground-high-1 hover:bg-background-high-1 flex-shrink-0" onClick={() => { setIsPanelMenuOpen(false); workspaceAction!.createNewPageInPanel(panelID) }}>
        ...
        <button type="button" className="flex px-2 h-10 bg-background text-foreground hover:text-foreground-high-1 hover:bg-background-high-1 flex-shrink-0" onClick={(e) => { setIsPanelMenuOpen(false); workspaceAction!.createNewDivision(panelID, "horizontal", "before"); e.stopPropagation() }}>
            <SplitLeft className="my-auto h-4 w-4" color={"var(--foreground-low-2)"} />
            <span className="my-auto mr-auto ml-2">Split Panel Left</span>
        </button>
        <button type="button" className="flex px-2 h-10 bg-background text-foreground hover:text-foreground-high-1 hover:bg-background-high-1 flex-shrink-0" onClick={(e) => { setIsPanelMenuOpen(false); workspaceAction!.createNewDivision(panelID, "horizontal", "after"); e.stopPropagation() }}>
        ...
        <button type="button" className="flex px-2 h-10 bg-background text-foreground hover:text-foreground-high-1 hover:bg-background-high-1 flex-shrink-0" onClick={(e) => { setIsPanelMenuOpen(false); workspaceAction!.createNewDivision(panelID, "vertical", "before"); e.stopPropagation() }}>
            <SplitTop className="my-auto h-4 w-4" color={"var(--foreground-low-2)"} />
            <span className="my-auto mr-auto ml-2">Split Panel Top</span>
        </button>
        <button type="button" className="flex px-2 h-10 bg-background text-foreground hover:text-foreground-high-1 hover:bg-background-high-1 flex-shrink-0" onClick={(e) => { setIsPanelMenuOpen(false); workspaceAction!.createNewDivision(panelID, "vertical", "after"); e.stopPropagation() }}>
...
function pageTabListScrollHandler(event: WheelEvent, containerElementRef: RefObject<HTMLDivElement>) {
    ...
    if (event.deltaX) return; // if scrolling is horizontal, scroll will be handled by the browser
